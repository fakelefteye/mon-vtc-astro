...
const transporter = nodemailer.createTransport({
    ...
    // Option pour aider à résoudre certains problèmes de connexion TLS sur Vercel
    tls: {
...
// Cette fonction est appelée par Stripe LORSQU'UN PAIEMENT EST RÉUSSI
export async function POST({ request }) {
    const signature = request.headers.get('stripe-signature');
    const body = await request.text();
    let event;

    // 1. Vérification de sécurité : on s'assure que la requête vient bien de Stripe
    try {
    ...
    // 2. Traitement de l'événement si c'est une confirmation de paiement
    if (event.type === 'checkout.session.completed') {
        const session = event.data.object;
        const data = session.metadata; // On récupère les données de la course (nom, email, etc.)
    ...
    // On confirme à Stripe que tout s'est bien passé
